...
def AsyncSelfPlay(game, args, iter_num, bar):
    ...
    for i in range(args.numPerProcessSelfPlay):
        ...
        bar.suffix = "iter:{i}/{x} | Total: {total:} | ETA: {eta:}".format(i=i + 1, x=args.numPerProcessSelfPlay,
        ...
        while True:
            ...
            if args.heuristic_type == 'combined':
                h_prob = heuristic2_prob(board)
                new_pi = (np.array(pi) + h_prob) / 2
                action = np.random.choice(len(new_pi), p=new_pi)
            elif args.heuristic_type == 'cooling':
                prob = args.heuristic_probability - (episodeStep - 1) * args.heuristic_probability / 42
                if np.random.ranf(1)[0] > prob:
                    action = np.random.choice(len(pi), p=pi)
                else:
                    action = args.heuristic_function(board)
            elif args.heuristic_type == 'normal':
                prob = args.heuristic_probability
                if np.random.ranf(1)[0] > prob:
                    action = np.random.choice(len(pi), p=pi)
                else:
                    action = args.heuristic_function(board)
            elif args.heuristic_type == 'default':
                action = np.random.choice(len(pi), p=pi)
            else:
                raise NameError("Wrong heuristic type '" + args.heuristic_type + "'")
...
def AsyncAgainst(game, args, iter_num, bar):
    bar.suffix = "iter:{i}/{x} | Total: {total:} | ETA: {eta:}".format(i=iter_num + 1, x=args.numAgainstPlayProcess,
                                                                       total=bar.elapsed_td, eta=bar.eta_td)
    bar.next()
